...
| **AUTH-MET-004** | AUTH | Sécurisation JWT : signature, claims et refresh tokens | **Done** | `AUTH-MET-001` | `Breizh360.Api/Services/ApiTokenService.cs`, `Breizh360.Api/Services/JwtOptions.cs`, `Breizh360.Api/Program.cs` | Mise en place d’un service générant un JWT signé (HMACSHA256), ajout des options typées pour la configuration et génération d’un refresh token aléatoire. Persistance et validation des refresh tokens à prévoir. |
| **MET-DOC-002** | META | Mise à jour des fichiers de suivi suite aux évolutions | **Done** | `MET-DOC-001` | `Breizh360.Metier/tasks.md` | Ce fichier a été mis à jour pour refléter la nouvelle étape `AUTH-MET-004`. |
