...
public static class AssemblyLoader
{
    public static Assembly LoadOrSkip(string assemblyName)
    {
        ...
        {
            throw SkipException.ForSkip($"Assembly '{assemblyName}' introuvable (ou non buildé). Détail: {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    public static Type GetTypeOrSkip(Assembly asm, string fullName)
    {
        ...
        if (t is null)
            throw SkipException.ForSkip($"Type '{fullName}' introuvable dans l'assembly '{asm.GetName().Name}'.");
        return t;
    }
}
